...
const boardCards = ref<CardChoice[]>([
  { rank: '7', suit: 'h' },
  { rank: '8', suit: 'h' },
  { rank: '2', suit: 'c' },
  { rank: '2', suit: 'c' },
...
const cardBlock = 'display: flex; align-items: center; gap: 0.25rem; padding: 0.35rem 0.5rem; border: 1px solid #ddd; border-radius: 6px; background: #fafafa; flex: 0 0 auto'
...
<template>
  <main>
    ...
    <form @submit.prevent="handleSubmit" style="display: flex; flex-direction: column; gap: 0.5rem; margin-top: 1rem">
      ...
      <div :style="handRow">
        <div v-for="(card, i) in heroCards" :key="i" :style="cardBlock">
          ...
          <span style="font-weight: 600; white-space: nowrap">
            <template v-if="card.rank && card.suit">{{ card.rank }}<span :style="isRedSuit(card.suit) ? { color: '#c00' } : {}">{{ SUIT_SYMBOLS[card.suit] }}</span></template>
      ...
      <div v-if="boardCount > 0" :style="boardRow">
        <div v-for="(card, i) in boardCards.slice(0, boardCount)" :key="i" :style="cardBlock">
          <select
            :value="card.rank"
            @change="setBoardCard(i, 'rank', ($event.target as HTMLSelectElement).value)"
          ...
          <select
            :value="card.suit"
            @change="setBoardCard(i, 'suit', ($event.target as HTMLSelectElement).value)"
      ...
      <label :style="labelBlock">
        <select v-model="preset" :style="selectStyle" aria-label="プリセット">
